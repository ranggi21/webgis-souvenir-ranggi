<?php
$id=$_GET['id'];
$sql = mysqli_query($conn,"SELECT small_industry.id,small_industry.name as nama,small_industry.address,small_industry.cp,small_industry.employee,small_industry.owner, status.status, industry_type.name,ST_AsText(small_industry.geom) as geom FROM small_industry join status on small_industry.id_status=status.id join industry_type on small_industry.id_industry_type=industry_type.id  where small_industry.id='$id'");
		...
		<div class="box box-primary"> -->
          ...
          <button class="btn btn-default my-btn" style="margin-top:10px" id="delete-button" type="button" title="Remove shape"><i class="fa fa-trash"></i></button></center>  </h3>
              ...
              <div class="col-lg-4 ds" id="hide3">
					...
					<div class="form-group" style="clear:both">
						<label for="nama">Place Name</label>
					...
					<div class="form-group">
						<label for="jns">Industry type</label>
						     <select required name="jns" id="jns" class="form-control">
						         <?php
					                                
					              $jns=mysqli_query($conn,"select * from industry_type ");
					              while($dt = mysqli_fetch_array($jns)){
